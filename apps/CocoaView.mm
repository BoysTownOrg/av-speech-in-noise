...
static constexpr auto textFieldLeadingEdge{labelWidth +
                                           labelToTextFieldSpacing};
...
static auto popUpButtonAtHeightWithWidth(CGFloat height, CGFloat width)
  ...
  return [[NSPopUpButton alloc]
      initWithFrame:NSMakeRect(textFieldLeadingEdge, height, width, labelHeight)
          pullsDown:NO];
}
...
CocoaTestSetupView::CocoaTestSetupView(NSRect r)
    : view_{[[NSView alloc] initWithFrame:r]},
      ...
      transducerMenu{[[NSPopUpButton alloc]
          initWithFrame:NSMakeRect(textFieldLeadingEdge, 90, menuWidth,
                                   labelHeight)
              pullsDown:NO]},
  ...
  const auto playCalibrationButton {
    button("play calibration", actions, @selector(playCalibration),
           NSMakeRect(width(r) - buttonWidth - reasonableSpacing -
                          1.5 * buttonWidth,
...
void CocoaSubjectView::addNumberButton(NSColor *color, int number, int row,
                                       std::size_t col) {
  auto title{asNsString(std::to_string(number))};
  const auto button {
    [NSButton buttonWithTitle:title target:actions action:@selector(respond:)]
  };
  ...
  auto attrString{[[NSAttributedString alloc] initWithString:title
...
void CocoaSubjectView::addNextTrialButton() {
  ...
  auto attrString{[[NSAttributedString alloc] initWithString:@"Press when ready"
                                                  attributes:attrsDictionary]};
...
constexpr auto leadingSecondaryTextEdge{leadingPrimaryTextEdge +
                                        primaryTextWidth + reasonableSpacing};
constexpr auto evaluationButtonsInnerGap{0};
constexpr auto evaluationButtonsWidth{2 * buttonWidth +
...
CocoaExperimenterView::CocoaExperimenterView(NSRect r)
    : view_{[[NSView alloc] initWithFrame:r]},
      ...
      secondaryDisplayedText_{[[NSTextField alloc]
          initWithFrame:NSMakeRect(
                            leadingSecondaryTextEdge, lowerPrimaryTextEdge(r),
      ...
      evaluationButtons{[[NSView alloc]
          initWithFrame:NSMakeRect(width(r) - evaluationButtonsWidth -
                                       buttonWidth,
                                   0, evaluationButtonsWidth, buttonHeight)]},
      ...
      responseSubmission{[[NSView alloc]
          initWithFrame:NSMakeRect(width(r) - responseSubmissionWidth, 0,
                                   ...
                                   buttonHeight + reasonableSpacing +
                                       2 * labelHeight + reasonableSpacing)]},
  ...
  [exitTestButton_ setFrame:NSMakeRect(0, height(r) - buttonHeight, buttonWidth,
                                       buttonHeight)];
  ...
  [nextTrialButton_ setFrame:NSMakeRect(width(r) - buttonWidth, 0, buttonWidth,
                                        buttonHeight)];
  ...
  [submitCorrectKeywords_
      setFrame:NSMakeRect(width(correctKeywordsSubmission.frame) - buttonWidth,
                          0, buttonWidth, buttonHeight)];
  ...
  addSubview(continueTestingDialog.contentView, continueTestingDialogMessage_);
...
static auto embeddedFrame(NSRect r) -> NSRect {
  return NSMakeRect(r.origin.x + windowPerimeterSpace,
                    r.origin.y + windowPerimeterSpace, innerWidth(r),
                    innerHeight(r));
}
...
CocoaView::CocoaView(NSRect r)
    : testSetup_{innerFrame(r)}, experimenter_{innerFrame(r)},
      ...
      deviceMenu{[[NSPopUpButton alloc]
          initWithFrame:NSMakeRect(textFieldLeadingEdge, 0, menuWidth,
                                   labelHeight)
              pullsDown:NO]},
      window{[[NSWindow alloc] initWithContentRect:r
