...
protected:
  ...
  RecognitionTestModel_Internal model{&targetPlayer, &maskerPlayer, &evaluator,
                                      &outputFile, &randomizer};
  ...
  void assertTargetVideoNotHidden() { assertFalse(targetPlayerVideoHidden()); }
  ...
  template <typename T>
  void assertDevicePassedToPlayer(const T &player,
  ...
  void assertThrowsRequestFailureWhenInvalidAudioDevice(
    ...
    useCase.setAudioDevice("a");
    assertCallThrowsRequestFailure(useCase, "'a' is not a valid audio device.");
  }
  ...
  void
  assertPassesCurrentTargetToEvaluatorBeforeAdvancingTarget(UseCase &useCase) {
    run(initializingTest);
...
TEST_F(RecognitionTestModel_InternalTests,
       targetPlaybackCompleteFadesOutMasker) {
  targetPlayer.playbackComplete();
  assertTrue(maskerPlayer.fadeOutCalled());
}

TEST_F(RecognitionTestModel_InternalTests,
       fadeOutCompleteNotifiesTrialComplete) {
  maskerFadeOutComplete();
...
TEST_F(RecognitionTestModel_InternalTests,
  ...
  assertCallThrowsRequestFailure(initializingTest,
...
TEST_F(RecognitionTestModel_InternalTests,
       playTrialDoesNotPlayIfTrialInProgress) {
  runIgnoringFailureWithTrialInProgress(playingTrial);
