...
public:
  ...
  void setTestInfo(const TestInformation &p) override { test.information = p; }
...
protected:
  ...
  WritingAdaptiveCoordinateResponseTrial writingAdaptiveCoordinateResponseTrial;
  ...
  std::string::size_type find_nth_element(const std::string &content, int n,
                                          char what) {
  ...
  std::string upUntilFirstOfAny(const std::string &content,
                                std::vector<char> v) {
...
TEST_F(OutputFileTests,
       writeOpenSetAdaptiveTrialTwiceDoesNotWriteHeadingTwice) {
  writeOpenSetAdaptiveTrial();
