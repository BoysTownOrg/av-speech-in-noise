...
std::vector<std::string> filesIn(DirectoryReader &reader,
                                 std::string directory = {}) {
  return reader.filesIn(std::move(directory));
}

std::vector<std::string> filter(FileFilter &filter_,
                                std::vector<std::string> files = {}) {
  return filter_.filter(std::move(files));
}

std::vector<std::string> subDirectories(DirectoryReader &reader,
                                        std::string directory = {}) {
...
protected:
  ...
  construct(std::vector<std::string> indentifiers = {}) {
    return FileIdentifierExcluderFilter{std::move(indentifiers)};
  }
};

TEST_F(FileIdentifierExcluderFilterTests,
       returnsFilesThatDontEndWithIdentifiers) {
