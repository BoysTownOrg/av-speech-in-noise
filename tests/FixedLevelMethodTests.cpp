...
protected:
  ...
  void submitCoordinateResponse() { method.submitResponse(coordinateResponse); }
  ...
  void setCurrentTarget(std::string s) { targetList.setCurrent(std::move(s)); }
...
TEST_F(FixedLevelMethodTests, completeQueryPassesTargetListToConcluder) {
  initialize();
  testComplete();
  assertEqual(static_cast<TargetList *>(&targetList),
