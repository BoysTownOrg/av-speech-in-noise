...
public:
  ...
  void run(RecognitionTestModel &model) override { model.initializeTest(test); }

  const CommonTest &commonTest() override { return test.common; }

  const TestInformation &testInformation() override { return test.information; }
...
protected:
  ...
  RecognitionTestModel model{&adaptiveMethod,     &fixedLevelMethod,
                             &infiniteTargetList, &fixedTrialTestConcluder,
                             &finiteTargetList,   &emptyTargetListTestConcluder,
                             &internalModel};
  ...
  void assertInitializesInternalModel(InitializingTestUseCase &useCase) {
    ...
    assertEqual(&useCase.testInformation(), internalModel.testInformation());
  }
};
