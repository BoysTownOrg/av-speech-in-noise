...
public:
  ...
  auto fixedLevelTestInitialized() const { return fixedLevelTestInitialized_; }
...
public:
  ...
  public:
    bool usingTargetsWithoutReplacement() override { return useFiniteTargets_; }

    void useFiniteTargets() { useFiniteTargets_ = true; }

    std::string trackSettingsFile() override { return trackSettingsFile_; }

    std::string calibrationLevel_dB_SPL() override { return calibrationLevel_; }
    ...
    std::string calibrationFilePath() override { return calibrationFilePath_; }
  ...
  public:
    ...
    auto responseSubmissionHidden() const { return responseSubmissionHidden_; }

    auto evaluationButtonsHidden() const { return evaluationButtonsHidden_; }

    auto nextTrialButtonShown() const { return nextTrialButtonShown_; }

    auto evaluationButtonsShown() const { return evaluationButtonsShown_; }

    auto responseSubmissionShown() const { return responseSubmissionShown_; }
...
public:
  explicit PlayingCalibration(ViewStub::TestSetupViewStub *view) : view{view} {}
...
public:
  ...
  std::string session(ModelStub &m) override { return information(m).session; }
  ...
  int floorSnr_dB(ModelStub &m) override { return adaptiveTest(m).floorSnr_dB; }
...
public:
  ...
  int snr_dB(ModelStub &m) override { return confirmingAdaptiveTest.snr_dB(m); }
...
public:
  ...
  bool nextTrialButtonShown() override { return view->nextTrialButtonShown(); }
...
public:
  ...
  bool responseViewShown() override { return view->responseSubmissionShown(); }
...
public:
  ...
  bool responseViewHidden() override { return view->evaluationButtonsHidden(); }
};
...
public:
  explicit BrowsingForMasker(ViewStub::TestSetupViewStub *view) : view{view} {}
...
protected:
  ...
  Presenter::Testing testing{&testingView};
  Presenter::Subject subject{&subjectView};
  Presenter presenter{&model,   &view,         &testSetup,
                      &subject, &experimenter, &testing};
  ...
  void assertExperimenterViewHidden() { assertTrue(experimenterViewHidden()); }
  ...
  void
  assertCancellingBrowseDoesNotChangePath(BrowsingEnteredPathUseCase &useCase) {
    useCase.setEntry("a");
  ...
  void setMaskerLevel(std::string s) { setupView.setMaskerLevel(std::move(s)); }
  ...
  void setStartingSnr(std::string s) { setupView.setStartingSnr(std::move(s)); }
  ...
  bool exitTestButtonShown() { return experimenterView.exitTestButtonShown(); }
  ...
  void assertCompleteTrialShowsResponseView(ConfirmingTestSetup &useCase,
