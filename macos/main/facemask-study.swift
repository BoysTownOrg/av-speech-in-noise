...
struct SwiftFacemaskStudyTestSetupView<Content: View>: View {
  ...
  var body: some View {
    if showing.value {
      VStack {
        HStack {
          Image("btnrh").resizable().aspectRatio(contentMode: .fit).frame(
            width: 318, height: 141, alignment: .topLeading
          ).background(Color(.white))
          Text("Facemask Study").font(.system(size: 48)).fixedSize(
            horizontal: true, vertical: false)
        }.padding()
        Form {
          TextField("Subject ID:", text: $subjectID_.string).font(.largeTitle).foregroundColor(
            .yellow)
            ...
            Text("START").font(.largeTitle)
              ...
              .background(
                RoundedRectangle(cornerRadius: 8).fill(
                  Color(
                    Color.RGBColorSpace.sRGB, red: 114 / 255, green: 172 / 255, blue: 77 / 255,
                    opacity: 1))
              )
              .frame(minWidth: 100)
          }
        ...
        Color(Color.RGBColorSpace.sRGB, red: 43 / 255, green: 97 / 255, blue: 198 / 255, opacity: 1)
      )
    }
  }
}
...
class SwiftFacemaskStudyTestSetupUI: NSObject, TestSetupUI {
  ...
  init(snrFunctor: @escaping () -> String) {
    testSettingsShortNames.items = [
      IdentifiableString(
        string: conditionShortName(
          stem: "NoMask_AO", testSettingsForShortName: &testSettingsForShortName)),
      IdentifiableString(
        string: conditionShortName(
          stem: "NoMask_AV", testSettingsForShortName: &testSettingsForShortName)),
      IdentifiableString(
        string: conditionShortName(
          stem: "CommunicatorMask_AO", testSettingsForShortName: &testSettingsForShortName)),
      IdentifiableString(
        string: conditionShortName(
          stem: "CommunicatorMask_AV", testSettingsForShortName: &testSettingsForShortName)),
    ]
...
class AppDelegate<Content: View>: NSObject, NSApplicationDelegate {
  ...
  init(snrFunctor: @escaping () -> String, @ViewBuilder snrView: @escaping () -> Content) {
    ...
    self.snrView = snrView
    AvSpeechInNoiseMain.facemaskStudy(
