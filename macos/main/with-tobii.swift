...
class EyeTrackerCalibrationValidationTesterUI: NSObject,
  AvSpeechInNoiseEyeTrackerCalibrationValidationTesterUI
{
...
class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  init(
    eyeTrackerRunMenu: SwiftEyeTrackerRunMenu,
    eyeTrackerCalibrationValidationTesterUI: EyeTrackerCalibrationValidationTesterUI
  ) {
  ...
  func applicationDidFinishLaunching(_: Notification) {
    ...
    AvSpeechInNoiseMain.withTobiiPro(
      SwiftTestSetupUIFactory(testSetupUI: testSetupUI), with: sessionUI, with: sessionUI.testUI,
      with: sessionUI.freeResponseUI, with: sessionUI.syllablesUI, with: sessionUI.chooseKeywordsUI,
      with: sessionUI.correctKeywordsUI, with: sessionUI.passFailUI,
      withEyeTrackerMenu: eyeTrackerRunMenu, with: eyeTrackerCalibrationValidationTesterUI)
    ...
    let contentView = SwiftSessionView(ui: sessionUI) {
      SwiftTestSetupView(
        ui: self.testSetupUI, testSettingsPathControl: self.testSettingsPathControl)
    ...
    if let screen = NSScreen.screens.first {
      window.setFrameOrigin(
        NSMakePoint((screen.frame.width - 480) / 2, (screen.frame.height - 300) / 2))
    }
  ...
  func applicationWillTerminate(_: Notification) {
    ...
    userDefaults.set(
      testSetupUI.testSettingsPathControl.url?.path ?? "", forKey: "TestSettingsFilePath")
...
class AppMenuReceiver: NSObject {
  ...
  func notifyThatRunEyeTrackerCalibrationValidationHasBeenClicked() {
    eyeTrackerCalibrationValidationTesterUI.observableObserver.observer?
      .notifyThatMenuHasBeenSelected()
  }
}
...
enum SwiftMain {
  static func main() {
    let eyeTrackerRunMenu = SwiftEyeTrackerRunMenu()
    let eyeTrackerCalibrationValidationTesterUI = EyeTrackerCalibrationValidationTesterUI()
    let appMenuReceiver = AppMenuReceiver(
      eyeTrackerRunMenu: eyeTrackerRunMenu,
      eyeTrackerCalibrationValidationTesterUI: eyeTrackerCalibrationValidationTesterUI)
    ...
    menuItemOne.submenu?.items = [
      NSMenuItem(
        title: "Quit \(applicationName)", action: #selector(NSApplication.terminate(_:)),
        keyEquivalent: "q")
    ]
    let legalMenu = NSMenuItem()
    legalMenu.submenu = NSMenu(title: "Legal")
    let aboutTobiiProMenuItem = NSMenuItem(
      title: "About Tobii Pro",
      action: #selector(AppMenuReceiver.notifyThatAboutTobiiProHasBeenClicked), keyEquivalent: "")
    ...
    let runEyeTrackerCalibration = NSMenuItem(
      title: "Eye Tracker Calibration",
      action: #selector(AppMenuReceiver.notifyThatRunEyeTrackerCalibrationHasBeenClicked),
      keyEquivalent: "")
    runEyeTrackerCalibration.target = appMenuReceiver
    let runEyeTrackerCalibrationValidation = NSMenuItem(
      title: "Eye Tracker Calibration Validation",
      action: #selector(AppMenuReceiver.notifyThatRunEyeTrackerCalibrationValidationHasBeenClicked),
      keyEquivalent: "")
