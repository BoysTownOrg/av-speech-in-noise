...
class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  func applicationDidFinishLaunching(_: Notification) {
    ...
    testSetupUI.testSettingsPathControl.url = URL(
      fileURLWithPath: userDefaults.string(forKey: "TestSettingsFilePath") ?? "")
...
enum SwiftMain {
  static func main() {
    ...
    case .denied:  // The user has previously denied access.
      terminatingAlert.informativeText =
        "User has denied microphone access. Check system privacy settings. Terminating."
      terminatingAlert.runModal()
      return

    case .restricted:  // The user can't grant access due to restrictions.
      terminatingAlert.informativeText =
        "User is restricted from granting microphone access. Terminating."
      terminatingAlert.runModal()
      return
    }
